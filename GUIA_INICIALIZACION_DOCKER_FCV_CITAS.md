...
```env
MYSQL_DATABASE=citas_fcv_training
...
Docker ejecuta `database/reference/db.sql` mediante
`/docker-entrypoint-initdb.d/01-db.sql` cuando crea `mysql_data` por primera
vez. Por ello deben aparecer las tablas y datos seed del modelo de referencia.

Esta ejecución solo ocurre con un volumen nuevo. Para volver a cargar el SQL
desde cero, ejecuta `docker compose down -v` y después
`docker compose up -d mysql`. Esto elimina los datos locales actuales.
